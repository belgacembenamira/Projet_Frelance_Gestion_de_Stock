...
import { getAllProducts, getProductStock, updateProduct, updateProductStock } from "./services/ProductService";
...
const Page: React.FC = () => {
  ...
  const enregistrerCommande = useCallback(async () => {
    try {
      const clientResponse = await createClient({
        ...
        amountRemaining: montantTotal - montantPartiel,
        amountPaid: montantPartiel, // Inclure le montant payé ici
      });

      // Affichage détaillé de la réponse du client
      ...
      // Créer la commande après la création du client
      ...
      console.log("Commande envoyée à la base de données ", commande);
      await createCommande(commande);

      // Mise à jour du stock des produits
      await Promise.all(
        articlesPanier.map(async (article) => {
          console.log(`Traitement de l'article : ${article.reference}`);

          // Récupérer la quantité en stock actuelle
          const stockQuantity = await getProductStock(article.id);
          console.log(`Quantité en stock pour l'article ${article.reference}: ${stockQuantity}`);

          // Quantité à déduire (quantité achetée par le client)
          const quantityToDeduct = quantitesPanier[article.id] ?? 0;
          console.log(`Quantité à déduire pour l'article ${article.reference}: ${quantityToDeduct}`);

          // Calcul de la nouvelle quantité de stock (ne jamais être inférieure à 0)
          const newQuantity = Math.max(stockQuantity - quantityToDeduct, 0);
          console.log(`Nouvelle quantité en stock pour l'article ${article.reference}: ${newQuantity}`);

          // Si la nouvelle quantité est négative, affiche un avertissement (au cas où cela arriverait)
          if (newQuantity < 0) {
            console.warn(`Stock insuffisant pour l'article ${article.reference}`);
          }

          // Mise à jour du stock dans la base de données
          console.log(`Mise à jour de la quantité en stock de l'article ${article.id} avec la nouvelle quantité: ${newQuantity}`);
          return updateProductStock(article.id, newQuantity);
        })
      );


      // Afficher un message de succès
      Swal.fire("Succès", "Commande enregistrée avec succès", "success");
    } catch (error) {
  ...
  const genererPDF = useCallback(async () => {
    try {
      ...
      doc.text("BON DE LIVRAISON", 195, 30, { align: "right" });
